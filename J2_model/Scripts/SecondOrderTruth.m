...
P(1) = 0;           % Covariance 
x1(1) = 0.98;       % SOC - Battery Fully Charged 
x2(1) = 0;          % Vc1
x3(1) = 0;          % Vc2
...
for k = 2:1:length(t)
    ...
    x1(k) = Ad(1,1)*x1(k-1) + Bd(1,1)*I(k-1)+ normrnd(0,0.003); % soc
    x2(k) = Ad(2,2)*x2(k-1) + Bd(2,1)*I(k-1)+ normrnd(0,0.0000002); % Vc1
    x3(k) = Ad(3,3)*x3(k-1) + Bd(3,1)*I(k-1)+ normrnd(0,0.0000002); % Vc2
    
    V_truth(k) = interp1(soc_intpts_OCV',OCV_intpts,x1(k-1))-I(k)*R0-x2(k)-x3(k)+normrnd(0,sqrt(R));
end 
...
save('SecondOrder_Truth_BestNAN','V','SOC_act','t','I'); 
